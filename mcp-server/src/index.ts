...
server.tool(
  "robot_connect",
  "Connect to the FTC robot Panels WebSocket on the Control Hub",
  {
...
server.tool(
  "robot_status",
  "Get robot agent snapshot: pose, estop, OpMode state, telemetry",
  {},
  async () => {
...
server.tool(
  "robot_start_opmode",
  "Initialize and start an OpMode via Panels OpModeControl",
  {
...
server.tool(
  "robot_stop",
  "Stop active OpMode and engage agent e-stop",
  {},
  async () => {
...
server.tool(
  "robot_drive",
  "Drive mecanum: x=strafe, y=forward, rotate=turn (-1 to 1). Commands expire after 200ms — call repeatedly to sustain.",
  {
...
server.tool(
  "robot_set_motor",
  "Set motor power or velocity. Names: fl, bl, fr, br, intake, launch, trans",
  {
    name: z.string(),
    power: z.number(),
...
server.tool(
  "robot_set_servo",
  "Set servo position (0-1). Names: hood, turret",
  {
    name: z.string(),
...
server.tool(
  "robot_set_config",
  "Tune a Panels Configurable field by id (live PID / tunables)",
  {
...
server.tool(
  "robot_get_telemetry",
  "Get latest agent telemetry snapshot from the robot",
  {},
  async () => {
...
server.tool(
  "robot_follow_path",
  "Follow a path in official FTC field coordinates (inches, center origin)",
  {
    waypoints: z
      .array(
        z.object({
...
server.tool(
  "robot_get_vision",
  "Get AprilTag vision detections from the agent snapshot",
  {},
  async () => {
    const snapshot = (await client.sendAgent("getSnapshot", {})) as {
      visionTags?: unknown;
    };
    return {
      content: [
...
server.tool(
  "robot_clear_estop",
  "Clear agent e-stop after a stop command",
  {},
  async () => {
