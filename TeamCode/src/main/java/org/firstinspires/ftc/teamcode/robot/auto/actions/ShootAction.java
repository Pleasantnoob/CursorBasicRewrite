...
/** Revs, aims, and feeds balls for a fixed duration. */
public class ShootAction implements Action {
    ...
    public ShootAction(RobotContainer robot, double goalX, double goalY, double durationSec) {
        this.robot = Objects.requireNonNull(robot, "robot");
        if (!robot.isInitialized()) {
            throw new IllegalArgumentException("RobotContainer must be initialized before creating ShootAction");
        }
        if (durationSec <= 0) {
    ...
    public boolean run(TelemetryPacket packet) {
        ...
        robot.shootSequence.runLauncherAuto(true);
